...
export function initCalendar(ctx) {
  ...
  function openEvent(id, el) {
    ...
    if (kind === 't') {
      ...
      popKind = 'event';
      E.pop.innerHTML = `<div class="cv-pop-h"><span class="lab">${t.state === 'scheduled' ? 'TAREA PROGRAMADA' : t.state.toUpperCase()}</span><span class="sp"></span><button class="cv-x" data-act="close">✕</button></div>
        <div class="cv-pop-t">${esc(t.title)}</div>
        <div class="cv-pop-m">${av(t.agent)} ${esc(a ? a.name : '')} · ${esc(DEPTS[t.dept].name)}${t.state === 'scheduled' ? ` · se ejecuta ${esc(untilText(t.dueAt))} (${hm(t.dueAt)})` : ''}${t.modelUsed ? ' · ' + esc(modelName(t.modelUsed)) : ''}</div>
        ${t.text && t.text !== t.title ? `<div class="cv-pop-p">${esc(t.text)}</div>` : ''}
        <div class="cv-row"><button class="cv-btn" data-act="open">ABRIR EL AGENTE</button>${t.state === 'scheduled' ? '<button class="cv-btn warn" data-act="cancel">CANCELARLA</button>' : ''}</div>`;
    ...
    popKind = 'event';
    E.pop.innerHTML = `<div class="cv-pop-h"><span class="lab">RUTINA</span><span class="sp"></span><button class="cv-x" data-act="close">✕</button></div>
      <div class="cv-pop-t">${esc(r.title)}</div>
      <div class="cv-pop-m">${av(r.agent)} ${esc(a ? a.name : r.agent)} · ${esc(r.desc || describe(r.when))}${r.needsOk ? ' · en espera de tu visto bueno' : ' · read-only'}${r.paused ? ' · <span class="cv-paused">PAUSADA</span>' : ''}</div>
      ...
      <div class="cv-row"><button class="cv-btn" data-act="run">EJECUTAR AHORA</button><button class="cv-btn" data-act="${r.paused ? 'resume' : 'pause'}">${r.paused ? 'REANUDAR' : 'PAUSAR'}</button><button class="cv-btn" data-act="only">SOLO ESTA</button><span class="sp"></span><button class="cv-btn warn" data-act="delete">ELIMINAR</button></div>`;
    E.pop.hidden = false; place(E.pop, el);
